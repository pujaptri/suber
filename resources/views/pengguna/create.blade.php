            ...
            <form action="{{ route('pengguna.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Andi Saputra" class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                        @error('name')
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email') }}" required placeholder="Contoh: andi@persuratan.go.id" class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                        @error('email')
                            <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <!-- Password -->
                    <div class="space-y-1.5">
                        <label for="password" class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Password</label>
                        <input type="password" name="password" id="password" required placeholder="Minimal 8 karakter" class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                        @error('password')
                            <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <!-- Konfirmasi Password -->
                    <div class="space-y-1.5">
                        <label for="password_confirmation" class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Konfirmasi Password</label>
                        <input type="password" name="password_confirmation" id="password_confirmation" required placeholder="Ulangi password" class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                    </div>
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <select name="role" id="role" required class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition cursor-pointer">
                            <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Admin (Staff Pengaju)</option>
                            <option value="superadmin" {{ old('role') == 'superadmin' ? 'selected' : '' }}>Superadmin (Pimpinan)</option>
                        </select>
                        @error('role')
                            <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                        @enderror
