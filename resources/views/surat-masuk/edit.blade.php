<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('surat-masuk.show', $surat->id) }}" class="inline-flex items-center gap-2 border border-gray-200 text-gray-600 hover:text-gray-900 hover:bg-gray-50 font-semibold py-2 px-4 rounded-xl text-sm transition duration-150 shadow-sm bg-white">
        ...
        <div class="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm hover:shadow-md transition duration-200">
            <form action="{{ route('surat-masuk.update', $surat->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <input type="hidden" name="tanggal_diterima" value="{{ $surat->tanggal_diterima->format('Y-m-d') }}">
                <input type="hidden" name="sifat_surat" value="{{ $surat->sifat_surat }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="kategori" id="kategori" required class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition cursor-pointer">
                            <option value="internal" {{ old('kategori', $surat->kategori) == 'internal' ? 'selected' : '' }}>Surat Dinas (Internal)</option>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="bagian" id="bagian" required class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition cursor-pointer">
                            <option value="Bagian Administrasi Umum" {{ old('bagian', $surat->bagian) == 'Bagian Administrasi Umum' ? 'selected' : '' }}>Bagian Administrasi Umum</option>
                            <option value="Bagian Kepegawaian" {{ old('bagian', $surat->bagian) == 'Bagian Kepegawaian' ? 'selected' : '' }}>Bagian Kepegawaian</option>
                            <option value="Bagian Keuangan" {{ old('bagian', $surat->bagian) == 'Bagian Keuangan' ? 'selected' : '' }}>Bagian Keuangan</option>
                        </select>
                        @error('bagian')
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="asal_surat" id="asal_surat" value="{{ old('asal_surat', $surat->asal_surat) }}" required class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                        @error('asal_surat')
                            <span class="text-xs text-red-500 font-semibold">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="perihal" id="perihal" rows="4" class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition leading-relaxed">{{ old('perihal', $surat->perihal) }}</textarea>
                    @error('perihal')
                        <span class="text-xs text-red-500 font-semibold">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-3" x-data="{ fileName: '', fileSize: '', hasFile: false, isDeleted: false, fileExtension: '{{ $surat->lampiran ? strtoupper(pathinfo($surat->lampiran, PATHINFO_EXTENSION)) : 'PDF' }}', isDragging: false, fileUrl: '{{ $surat->lampiran ? '/storage/' . $surat->lampiran : '' }}' }">
                    ...
                    <!-- Selected File List (Mockup fidelity) -->
                    <div class="flex items-center justify-between p-4 border border-gray-100 rounded-xl bg-slate-50/30" x-show="(hasFile || {{ $surat->lampiran ? 'true' : 'false' }}) && !isDeleted">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h5 class="text-sm font-bold text-gray-800 truncate max-w-[200px]" x-text="hasFile ? fileName : '{{ $surat->lampiran ? preg_replace('/^\d+_/', '', basename($surat->lampiran)) : 'surat_masuk_03_original.pdf' }}'">surat_masuk_03_original.pdf</h5>
                                <p class="text-[11px] font-semibold text-gray-400" x-text="hasFile ? fileSize : '{{ ($surat->lampiran && Storage::disk('public')->exists($surat->lampiran)) ? number_format(Storage::disk('public')->size($surat->lampiran) / (1024 * 1024), 1) . ' MB' : '1.2 MB' }}'"></p>
                            </div>
                        </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-50">
                    <a href="{{ route('surat-masuk.show', $surat->id) }}" class="border border-gray-200 text-gray-600 hover:text-gray-900 hover:bg-gray-50 font-bold py-3 px-8 rounded-xl text-sm transition duration-150 shadow-sm text-center">
                        Batal
                    </a>
