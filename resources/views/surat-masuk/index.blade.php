<x-app-layout>
    ...
    <div class="space-y-6" x-data="{ deleteModalOpen: false, deleteActionUrl: '', deleteNomorSurat: '' }">
        ...
        <div class="flex items-center justify-between">
            <h3 class="text-2xl font-bold text-gray-800 tracking-tight">Daftar Surat Masuk</h3>
            <div class="flex items-center gap-3">
                <!-- Tambah Surat -->
                <a href="{{ route('surat-masuk.create') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-5 rounded-xl text-sm transition duration-150 shadow-md shadow-blue-100">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <!-- Filter Card -->
        <form method="GET" action="{{ route('surat-masuk.index') }}" class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                    ...
                    <div class="md:col-span-1 flex gap-3">
                        ...
                        <a href="{{ route('surat-masuk.index') }}" class="flex-1 border border-blue-600 text-blue-600 hover:bg-blue-50 font-semibold py-2.5 px-4 rounded-xl text-sm transition duration-150 shadow-sm text-center">
                            Reset
                        </a>
                    </div>
                </div>
        ...
        <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition duration-200">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-gray-50 text-gray-400 text-xs font-bold uppercase tracking-wider">
                            ...
                            <th class="py-4.5 px-4">Pengirim</th>
                            ...
                            @foreach($suratMasuk as $index => $surat)
                                <tr class="hover:bg-slate-50/30 transition">
                                    <td class="py-4 px-4 text-center text-sm font-semibold text-gray-500">
                                        {{ ($suratMasuk->currentPage() - 1) * $suratMasuk->perPage() + $index + 1 }}
                                    </td>
                                    ...
                                    <td class="py-4 px-4 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <!-- View -->
                                            <a href="{{ route('surat-masuk.show', $surat->id) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 border border-blue-100 rounded-lg transition" title="Lihat">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            ...
                                            <!-- Edit -->
                                            <a href="{{ route('surat-masuk.edit', $surat->id) }}" class="p-1.5 text-gray-500 hover:bg-gray-50 border border-gray-200 rounded-lg transition" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                                </svg>
                                            </a>
                                            <!-- Delete -->
                                            <button type="button" @click.prevent="deleteModalOpen = true; deleteActionUrl = '{{ route('surat-masuk.destroy', $surat->id) }}'; deleteNomorSurat = '{{ $surat->nomor_surat }}';" class="p-1.5 text-red-600 hover:bg-red-50 border border-red-100 rounded-lg transition" title="Hapus">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <div class="px-6 py-4 bg-slate-50/30 border-t border-gray-50 flex items-center justify-between flex-wrap gap-4">
                <span class="text-xs font-semibold text-gray-400">
                    @if($suratMasuk->isNotEmpty())
                        Menampilkan {{ $suratMasuk->firstItem() }} sampai {{ $suratMasuk->lastItem() }} dari {{ $suratMasuk->total() }} data
                    @else
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    @else
                        ...
                        <button type="button" class="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-semibold border border-transparent text-gray-500 hover:bg-gray-50 transition">25</button>
