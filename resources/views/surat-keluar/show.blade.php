<x-app-layout>
    ...
    @php
        ...
        if ($sifat === 'penting') {
            $sifatBadgeColor = 'bg-amber-50 text-amber-600 border border-amber-100';
            $sifatBadgeText = 'PENTING';
        } elseif ($sifat === 'rahasia' || $sifat === 'sangat_rahasia') {
            $sifatBadgeColor = 'bg-red-50 text-red-600 border border-red-100';
            $sifatBadgeText = strtoupper(str_replace('_', ' ', $sifat));
        } else {
            $sifatBadgeColor = 'bg-blue-50 text-blue-600 border border-blue-100';
            $sifatBadgeText = 'BIASA';
        }

        // Define status badge classes
        $status = strtolower($surat->status);
        if ($status === 'disetujui') {
            $statusBadgeColor = 'bg-green-50 text-green-600 border border-green-100';
            $statusBadgeText = 'DISETUJUI';
        } elseif ($status === 'ditolak') {
            $statusBadgeColor = 'bg-red-50 text-red-600 border border-red-100';
            $statusBadgeText = 'DITOLAK';
        } elseif ($status === 'revisi') {
            $statusBadgeColor = 'bg-orange-50 text-orange-600 border border-orange-100';
            $statusBadgeText = 'REVISI';
        } else {
            $statusBadgeColor = 'bg-amber-50 text-amber-600 border border-amber-100';
            $statusBadgeText = 'MENUNGGU PERSETUJUAN';
        }
    @endphp
    ...
    <div class="space-y-6" x-data="{ previewModalOpen: false, previewUrl: '', previewType: '', previewTitle: '' }">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200 relative">
                    ...
                    <div class="absolute top-6 right-6 flex items-center gap-2">
                        <span class="text-xs font-bold px-3 py-1 rounded-lg {{ $sifatBadgeColor }}">
                            {{ $sifatBadgeText }}
                        </span>
                        <span class="text-xs font-bold px-3 py-1 rounded-lg {{ $statusBadgeColor }}">
                            {{ $statusBadgeText }}
                        </span>
                    </div>
                    ...
                    <div class="space-y-1 mb-6">
                        <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Nomor Surat Keluar</span>
                        <h3 class="text-3xl font-extrabold text-gray-900 mt-1 leading-tight">{{ $surat->nomor_surat }}</h3>
                        <p class="text-sm text-gray-500 font-medium">Tanggal Surat: {{ $surat->tanggal_surat->locale('id')->isoFormat('DD MMM Y') }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 mb-6">
                        ...
                        <div class="space-y-1.5">
                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Penerima Surat</span>
                            ...
                            <span class="text-sm text-gray-500 font-medium block">
                                Nama Jabatan / Individu Penerima
                            </span>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <span class="text-base font-bold text-gray-800 block capitalize">
                                {{ $surat->kategori }}
                            </span>
                            <span class="text-sm text-gray-500 font-medium block">
                                Kelompok Dokumen
                        ...
                        <!-- Sifat Surat -->
                        <div class="space-y-1.5">
                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Sifat Surat</span>
                            <span class="text-base font-bold text-gray-800 block capitalize">
                                {{ str_replace('_', ' ', $surat->sifat_surat) }}
                            </span>
                            <span class="text-sm text-gray-500 font-medium block">
                                Derajat Keamanan
                            </span>
                        ...
                        <div class="space-y-1.5">
                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Status Persetujuan</span>
                            <span class="text-base font-bold text-gray-800 block">
                                @if($surat->approvedBy)
                                    Disetujui oleh {{ $surat->approvedBy->name }}
                                @else
                                    Belum Disetujui
                                @endif
                            </span>
                    ...
                    <!-- Catatan Revisi block if exists -->
                    @if($surat->catatan_revisi)
                        <div class="bg-orange-50 border border-orange-100 rounded-xl p-4 mb-6">
                            <span class="text-xs font-extrabold text-orange-700 uppercase tracking-wider block mb-1">Catatan Revisi / Penolakan</span>
                            <p class="text-sm text-orange-800 leading-relaxed font-semibold italic">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1.5">
                            ...
                            <h4 class="text-lg font-bold text-gray-805 capitalize">{{ str_replace('_', ' ', $surat->kategori) }}</h4>
                        </div>
                        ...
                        @if($surat->isi_surat)
                            <div class="space-y-1.5 pt-2">
                                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Isi Ringkas Surat</span>
                                <div class="bg-slate-50/50 p-4 rounded-xl border border-gray-100 text-gray-650 text-sm leading-relaxed whitespace-pre-line font-medium">
                                    {{ $surat->isi_surat }}
                                </div>
                            </div>
                        @endif
                        ...
                        @if($surat->keterangan)
                            <div class="space-y-1.5 pt-2">
                                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Keterangan</span>
                                <p class="text-sm text-gray-500 font-medium">{{ $surat->keterangan }}</p>
                            </div>
                        @endif
                    </div>
                </div>

                <!-- Riwayat Review TTD Card -->
                <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200">
                    <div class="flex items-center justify-between pb-4 border-b border-gray-50 mb-6">
                        <h4 class="text-lg font-bold text-gray-800">Riwayat Review & Persetujuan</h4>
                    ...
                    <div class="relative pl-6 space-y-6 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-px before:bg-gray-100">
                        @if($surat->reviews->isNotEmpty())
                            @foreach($surat->reviews as $review)
                                @php
                                    $revKeputusan = strtolower($review->keputusan);
                                    if ($revKeputusan === 'disetujui') {
                                        $revDotColor = 'border-green-600 bg-white';
                                        $revBadgeColor = 'bg-green-50 text-green-600';
                                        $revBadgeText = 'DISETUJUI';
                                    } elseif ($revKeputusan === 'ditolak') {
                                        $revDotColor = 'border-red-600 bg-white';
                                        $revBadgeColor = 'bg-red-50 text-red-600';
                                        $revBadgeText = 'DITOLAK';
                                    } else {
                                        $revDotColor = 'border-orange-500 bg-white';
                                        $revBadgeColor = 'bg-orange-50 text-orange-600';
                                        $revBadgeText = 'REVISI';
                                    }
                                @endphp
                                <div class="relative">
                                    <div class="absolute -left-6 top-1.5 w-4.5 h-4.5 rounded-full bg-white border-4 {{ $revDotColor }}"></div>
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2">
                                            <h5 class="text-sm font-bold text-gray-800">{{ $review->reviewer->name }}</h5>
                                            <span class="text-[9px] font-extrabold px-2 py-0.5 rounded {{ $revBadgeColor }}">{{ $revBadgeText }}</span>
                                        </div>
                                        <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">
                                            {{ $review->reviewed_at->locale('id')->isoFormat('DD MMM Y • HH:mm') }}
                                        </p>
                                        <p class="text-sm text-gray-600 mt-1 italic font-medium">"{{ $review->catatan }}"</p>
                                        @if($review->tanda_tangan)
                                            <div class="mt-2.5 max-w-[120px] p-1.5 border border-gray-100 rounded-lg bg-gray-50/50">
                                                <img src="/storage/{{ $review->tanda_tangan }}" alt="Tanda Tangan" class="max-h-16 object-contain mix-blend-multiply">
                                            </div>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="relative">
                            <div class="absolute -left-6 top-1.5 w-4.5 h-4.5 rounded-full bg-white border-4 border-gray-300"></div>
                            <div class="space-y-1">
                                <h5 class="text-sm font-bold text-gray-800">Surat Dibuat</h5>
                                <p class="text-xs text-gray-400 font-semibold uppercase tracking-wider">
                                    {{ $surat->created_at->locale('id')->isoFormat('DD MMM Y • HH:mm') }} oleh {{ $surat->user->name ?? 'Staf Admin' }}
                                </p>
                                <p class="text-sm text-gray-500">Menunggu persetujuan SuperAdmin.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <!-- Lampiran Widget -->
                <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200">
                    <div class="flex items-center justify-between pb-4 border-b border-gray-55 mb-4">
                        <h4 class="text-xs font-bold text-gray-805 uppercase tracking-wider">Lampiran Dokumen</h4>
                        <div class="text-gray-400">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200">
                    ...
                    <div class="flex items-center gap-3.5 p-3 border border-gray-100 rounded-xl">
                        ...
                        <div class="flex flex-col">
                            <span class="text-sm font-bold text-gray-800 leading-none">{{ $surat->user->name ?? 'Staf Admin' }}</span>
                            <span class="text-xs font-medium text-gray-400 mt-1 capitalize">{{ $surat->user->role ?? 'Operator' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200">
                    ...
                    <div class="space-y-3">
                        ...
                        @if(Auth::user()->isSuperAdmin() || in_array($surat->status, ['menunggu_ttd', 'revisi']))
                            <a href="{{ route('surat-keluar.edit', $surat->id) }}" class="w-full inline-flex items-center justify-center gap-2 border border-blue-600 text-blue-600 hover:bg-blue-50 font-bold py-3.5 px-4 rounded-xl text-sm transition duration-150 shadow-sm text-center uppercase">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @else
                            <button type="button" disabled class="w-full inline-flex items-center justify-center gap-2 border border-gray-150 text-gray-300 font-bold py-3.5 px-4 rounded-xl text-sm cursor-not-allowed opacity-60 text-center uppercase">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                </svg>
                                Edit Data
                        ...
                        @if(Auth::user()->isSuperAdmin() || $surat->status !== 'disetujui')
                            <form action="{{ route('surat-keluar.destroy', $surat->id) }}" method="POST" class="w-full" onsubmit="return confirm('Apakah Anda yakin ingin menghapus surat keluar ini secara permanen?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full inline-flex items-center justify-center gap-2 border border-red-650 text-red-650 hover:bg-red-50 font-bold py-3.5 px-4 rounded-xl text-sm transition duration-150 shadow-sm text-center uppercase">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <!-- SuperAdmin Signoff/Persetujuan Panel -->
                @if(Auth::user()->isSuperAdmin() && $surat->status === 'menunggu_ttd')
                    <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200">
                        <span class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 block">Persetujuan Surat</span>

                        <div class="space-y-4">
                            <!-- Approve Form -->
                            <form action="{{ route('surat-keluar.setujui', $surat->id) }}" method="POST" class="space-y-3.5">
                                @csrf
                                <div class="space-y-1">
                                    <label class="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Catatan Setuju (Opsional)</label>
                                    <textarea name="catatan" rows="2" placeholder="Disetujui..." class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl p-2.5 text-xs text-gray-700 transition"></textarea>
                                </div>
                                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs uppercase tracking-wider transition duration-150 text-center shadow-sm">
                                    Setujui
                                </button>
                            </form>

                            <!-- Reject/Request Revision Form -->
                            <form action="{{ route('surat-keluar.tolak', $surat->id) }}" method="POST" class="space-y-3.5 border-t border-gray-100 pt-4">
                                @csrf
                                <div class="space-y-1">
                                    <label class="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Catatan Penolakan/Revisi (Wajib)</label>
                                    <textarea name="catatan" required rows="2" placeholder="Harap lengkapi nomor / lampiran..." class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl p-2.5 text-xs text-gray-700 transition"></textarea>
                                </div>
                                <div class="space-y-1">
                                    <label class="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Tindakan Keputusan</label>
                                    <select name="keputusan" required class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl p-2 text-xs text-gray-700 transition cursor-pointer">
                                        <option value="revisi">Minta Revisi</option>
                                        <option value="ditolak">Tolak Surat</option>
                                    </select>
                                </div>
                                <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs uppercase tracking-wider transition duration-150 text-center shadow-sm">
                                    Proses Tolak / Revisi
                                </button>
                            </form>
                        </div>
