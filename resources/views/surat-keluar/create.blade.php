        ...
        <div class="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm hover:shadow-md transition duration-200">
            <form action="{{ route('surat-keluar.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="kategori" id="kategori" required class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition cursor-pointer">
                            <option value="internal" {{ old('kategori') == 'internal' ? 'selected' : '' }}>Internal (Dinas)</option>
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <input type="text" name="penerima" id="penerima" value="{{ old('penerima') }}" placeholder="Contoh: Kepala Bagian Umum / Nama Penerima" required class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                        @error('penerima')
                            <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="perihal" id="perihal" rows="3" placeholder="Tuliskan perihal atau judul utama surat..." class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition leading-relaxed">{{ old('perihal') }}</textarea>
                    @error('perihal')
                        <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="isi_surat" id="isi_surat" rows="5" placeholder="Tuliskan intisari isi surat keluar di sini..." class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition leading-relaxed">{{ old('isi_surat') }}</textarea>
                    @error('isi_surat')
                        <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="2" placeholder="Catatan internal atau keterangan lain..." class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition leading-relaxed">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-3" x-data="{ fileName: '', fileSize: '', hasFile: false, fileExtension: 'PDF', isDragging: false, fileUrl: '' }">
                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Unggah Lampiran Dokumen <span class="text-red-500">*</span></span>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-50">
                    <a href="{{ route('surat-keluar.index') }}" class="border border-gray-200 text-gray-600 hover:text-gray-900 hover:bg-gray-50 font-bold py-3 px-8 rounded-xl text-sm transition duration-150 shadow-sm text-center">
                        Batal
                    </a>
