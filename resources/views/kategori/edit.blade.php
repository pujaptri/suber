            ...
            <form action="{{ route('kategori.update', $kategori->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama', $kategori->nama) }}" required placeholder="Contoh: Nota Dinas" class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                        @error('nama')
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="kode" id="kode" value="{{ old('kode', $kategori->kode) }}" required placeholder="Contoh: CAT-001" class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition">
                        @error('kode')
                            <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4" placeholder="Tuliskan deskripsi lengkap mengenai pengarsipan kategori ini..." class="w-full bg-slate-50 border border-gray-100 focus:border-blue-500 focus:bg-white focus:ring-0 rounded-xl py-3 px-4 text-sm text-gray-700 transition leading-relaxed">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <span class="text-xs text-red-500 font-semibold block mt-1">{{ $message }}</span>
                    @enderror
                </div>
